L.TileLayer.MaskCanvas = L.TileLayer.Canvas.extend({
    ...
    setData: function(dataset) {
        ...
        // Get bounds of the points for the overlay
        this.bounds = new L.LatLngBounds(dataset);
        // Create a new quadtree that contains all of the points, quadtree takes the bounds of the dataset as an object
        // with params x: southwest coord, y: southwest coord, width: northeast coord, height: northeast coord
    ...
    _drawPoints: function (ctx, coordinates) {
        // Create canvas object for overlay tiles to draw masking layer
        var c = ctx.canvas,
            g = c.getContext('2d'),
            self = this,
            p,
        ...
        // globalCompositeOperation is a built-in canvas feature that causes the transparency of layers to be
        // adjusted when adding more layers of a certain mask composition, in this case, destination-out
        g.globalCompositeOperation = 'source-over';
        if (!this.options.noMask) {
            g.fillRect(0, 0, tileSize, tileSize);
            g.globalCompositeOperation = 'destination-out';
        }
        // coordinates.forEach(function(coords) {  // Original loop, optimized to standard for loop
